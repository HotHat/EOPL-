(module lang (lib "eopl.ss" "eopl")
  ...
  (define the-grammar
    '((program (expression) a-program)
      ...
      (expression 
       ("cond" cond-items "end") cond-exp)
      (cond-items
       (expression "==>" expression cond-items) cond-item-exp)
      (cond-items () empty-cond-exp)
  ...
  (define value-of-cond
    (lambda (val env)
      (cases cond-items val
        (cond-item-exp (exp1 exp2 items)
           (let ((val1 (value-of exp1 env)))
                 
             (if (expval->bool val1)
                 (value-of exp2 env)
                 (value-of-cond items env))))
                 
        (empty-cond-exp ()
            (eopl:printf "Error in cond expression!")))))
  ...
  (eopl:printf "~a~%" (run "let x = 6 in cond equal?(x,6)==>15 equal?(x,5)==>9 end"))
